...
Foundation.prototype.Build = function(builderEnt, work)
{
	...
	if (progress >= 1.0)
	{
		...
		*/
		...
		let cmpBuildingIdentity = Engine.QueryInterface(building, IID_Identity);
		for (let unit of cmpBuildingIdentity.GetSpawnUnitList())
			this.SpawnUnit(unit, building);
		...
		Engine.DestroyEntity(this.entity, building);
	}
};

Foundation.prototype.SpawnUnit = function(unitCode, building)
{
	let cmpIdentity = Engine.QueryInterface(building, IID_Identity);
	let civ_code = cmpIdentity.GetCiv();
	let templateName = "units/"+civ_code+"_"+unitCode;

	let cmpOwnership = Engine.QueryInterface(building, IID_Ownership);
	let owner = cmpOwnership.GetOwner();

	let newEntity = Engine.AddEntity(templateName);
	let cmpEntityOwnership = Engine.QueryInterface(newEntity, IID_Ownership);
	cmpEntityOwnership.SetOwner(owner);

	let cmpFootprint = Engine.QueryInterface(building, IID_Footprint);
	let pos = cmpFootprint.PickSpawnPoint(newEntity);
	let cmpEntityPosition = Engine.QueryInterface(newEntity, IID_Position);
	cmpEntityPosition.JumpTo(pos.x, pos.z);
}
