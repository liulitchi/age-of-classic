...
Damage.prototype.MissileHit = function(data, lateness)
{
	...
	{
		data.multiplier = GetDamageBonus(data.target, data.bonus, data.type, data.attacker);
	...
	for (let ent of ents)
	{
		...
		this.CauseDamage({
			...
			"target": ent,
			"attacker": data.attacker,
			"multiplier": GetDamageBonus(ent, data.bonus, data.type, data.attacker),
			"type": data.type,
			"attackerOwner": data.attackerOwner
		});
		cmpProjectileManager.RemoveProjectile(data.projectileId);
...
Damage.prototype.CauseSplashDamage = function(data)
{
	...
	for (let ent of nearEnts)
	{
		...
		if (data.splashBonus)
			damageMultiplier *= GetDamageBonus(ent, data.splashBonus, data.type, data.attacker);
